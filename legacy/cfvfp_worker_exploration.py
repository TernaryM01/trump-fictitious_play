...
class GameTraversalWorker:
    def __init__(self, args: Dict):
        ...
        self.branching_rate = args['branching_rate']
        self.branching_depth_limit = args['branching_depth_limit']
    ...
    def _traverse_game_tree(self, state: pyspiel.State, traverser_player: int, branching_depth: int) -> float:
        if state.is_terminal():
            ...
            return self._traverse_game_tree(state, traverser_player, branching_depth)
        ...
        if active_player == traverser_player:
            ...
            if branching_depth > 0:
                self.rng_key, rng_subkey = jax.random.split(self.rng_key)
                n_branches = jax.random.poisson(rng_subkey, self.branching_rate, shape=())
                n_branches = int(n_branches)

                if n_branches > 0:
                    legal_actions = state.legal_actions(active_player)
                    if n_branches + 1 >= len(legal_actions):
                        exploratory_actions = [action for action in legal_actions if action != best_response_item]
                    else:
                        self.rng_key, rng_subkey = jax.random.split(self.rng_key)
                        exploratory_actions = jax.random.choice(rng_subkey, jnp.array(legal_actions), shape=(n_branches,), replace=False)
                        exploratory_actions = [action.item() for action in exploratory_actions if action != best_response_item]

                    for action in exploratory_actions:
                        self.num_traversals -= 1  # Decrease number of traversals every time a new branch is launched.

                        achieved_value_exploratory = self._traverse_game_tree(state.child(action), traverser_player, branching_depth - 1)
            ...
            achieved_value = self._traverse_game_tree(state, traverser_player, branching_depth)
            self.q_value_target_memories_p[phase].append({
                'info_state': infostate_q.astype(jnp.float16),
    ...
    def run(self) -> Optional[Tuple[List[int], List[int]]]:
        ...
        while self.num_traversals > 0:
            self.num_traversals -= 1
            try:
                self._traverse_game_tree(root_state.clone(), self.player, self.branching_depth_limit)
            except Exception as e:
                import traceback
                print(f"Worker {self.worker_id}: A traversal failed with error: {e}")
