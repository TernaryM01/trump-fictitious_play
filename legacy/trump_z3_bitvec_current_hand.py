...
def analyze_cards_with_z3(player_cards, graveyards, current_trick_opp_cards, leading_suit, trump_suit,
    ...
    def add_current_hand_and_played_cards_domain_constraints(solver, opponent_hands, opponent_plays, trick_num):
        ...
        for opp in range(NUM_PLAYERS - 1):
            ...
            num_cards_hand_constraint_sumif = z3.Sum([z3.If((opponent_hands[opp] & (1 << card)) != 0, 1, 0)
                                                for card in range(NUM_CARDS)]) \
                                                    == NUM_CARDS_PER_PLAYER + 1 - trick_num
            solver.add(num_cards_hand_constraint_sumif)
            num_cards_hand_constraint_pb = z3.PbEq([(opponent_hands[opp] & (1 << card) != 0, 1)
                                                 for card in range(NUM_CARDS)],
                                                NUM_CARDS_PER_PLAYER + 1 - trick_num)
        ...
        for card in range(NUM_CARDS):
            # Two equivalent versions are both used
            no_two_owners_constraint_sumif = z3.Sum([z3.If(opponent_hands[opp] & (1 << card) != 0, 1, 0)
                                               for opp in range(NUM_PLAYERS - 1)]) <= 1
            solver.add(no_two_owners_constraint_sumif)
            no_two_owners_constraint_pb = z3.PbLe([(opponent_hands[opp] & (1 << card) != 0, 1)
                                                for opp in range(NUM_PLAYERS - 1)], 1)
    ...
    def add_legality_constraints(solver, opponent_hands, opponent_plays,
        ...
        for opp in range(NUM_PLAYERS - 1):
            ...
            # for card in leading_suit_cards:
            #     leading_mask |= (1 << card)
            leading_mask = ((1 << NUM_CARDS_PER_SUIT) - 1) << (leading_suit * NUM_CARDS_PER_SUIT)
            has_leading_suit = (opponent_hands[opp] & leading_mask) != 0
            ...
            if not break_occurred and leading_suit != trump_suit:
                ...
                # for card in [card for card in range(NUM_CARDS) if card not in trump_cards]:
                #     available_nontrump_mask |= (1 << card)
                trump_mask = ((1 << NUM_CARDS_PER_SUIT) - 1) << (trump_suit * NUM_CARDS_PER_SUIT)
                available_nontrump_mask = ((1 << NUM_CARDS) - 1) & ~trump_mask
    ...
    def add_hand_bool_bitvec_eq_constraints(solver, opponent_hands, opponent_hand_cards):
        """Enforce equivalence between bit-vector and Boolean array representations of opponent hands."""
        for opp in range(NUM_PLAYERS - 1):
            for card in range(NUM_CARDS):
                # Bit-vector representation: check if bit at position 'card' is set
                bitvec_has_card = (opponent_hands[opp] & (1 << card)) != 0
                # Boolean array representation: direct Boolean variable
                bool_has_card = opponent_hand_cards[opp][card]
                # Enforce equivalence: bitvec_has_card ↔ bool_has_card
                equivalence_constraint = z3.And(
                    z3.Implies(bitvec_has_card, bool_has_card),
                    z3.Implies(bool_has_card, bitvec_has_card)
                )
                solver.add(equivalence_constraint)
    ...
    if debug:
        opponent_hand_cards = [[z3.Bool(f"opp_{opp}_has_card_{card}") for card in range(NUM_CARDS)]
                               for opp in range(NUM_PLAYERS - 1)]
    ...
    if debug: add_hand_bool_bitvec_eq_constraints(solver, opponent_hands, opponent_hand_cards)
